...
class TransportController(BaseController):
    ...
    def __init__(
        ...
        self.loop = _PadLoopController(self)
        self.playback = _PadPlaybackController(self)

    def _apply_project_state_to_audio(self) -> None:
        apply_project_state = _ApplyProjectState(self, self._project, self._session, self._audio)
...
class _ApplyProjectState:
    ...
    def _apply_pad_loop_regions(self, defaults: ProjectState) -> None:
        for sample_id in range(len(self._project.sample_paths)):
            ...
            self._transport._apply_effective_pad_loop_region_to_audio(sample_id)
    ...
    def _apply_pad_bpm_settings(self) -> None:
        for sample_id in range(len(self._project.sample_paths)):
            if (
                ...
                continue
            self._transport._on_pad_bpm_changed(sample_id)

    def _apply_bpm_lock_settings(self) -> None:
        if self._project.bpm_lock:
            anchor_pad_id = self._project.selected_pad
            anchor_bpm = normalize_bpm(self._transport.effective_bpm(anchor_pad_id))
            self._session.bpm_lock_anchor_pad_id = anchor_pad_id
            self._session.bpm_lock_anchor_bpm = anchor_bpm
        else:
            self._session.bpm_lock_anchor_pad_id = None
        ...
        self._transport._recompute_master_bpm()


class _PadPlaybackController:
    ...
    def toggle(self, sample_id: int) -> None:
        ...
        if sample_id in self._transport._session.active_sample_ids:
            self._transport.stop_pad(sample_id)
        else:
            self.play(sample_id)


class _PadLoopController:
    ...
    def reset(self, sample_id: int) -> None:
        self._transport._reset_pad_loop_region(sample_id)

    def effective_region(self, sample_id: int) -> tuple[float, float | None]:
        validate_sample_id(sample_id)
        return self._transport._effective_pad_loop_region(sample_id)
    ...
    def set_auto(self, sample_id: int, *, enabled: bool) -> None:
        ...
        if enabled:
            ...
            start_s = self._transport._snap_to_nearest_beat(start_s, beats)
            start_s = self._transport._quantize_time_to_output_samples(start_s)
    ...
    def set_end(self, sample_id: int, end_s: float | None) -> None:
        ...
        if end_s is not None:
            ensure_finite(end_s)
            end_s = max(0.0, float(end_s))
            end_s = self._transport._quantize_time_to_output_samples(end_s)

            start_s = self._transport._quantize_time_to_output_samples(
