...
def main():
    ...
    for i in range(GRID_SIZE):
        for j in range(GRID_SIZE):
            ...
            stem_indicator.bind(
                "<Button-1>",
                lambda e, b=btn_id: select_stems_button(b, update_stem_buttons_state_impl),
            )
    ...
    speed_slider = tk.Scale(
        ...
        command=lambda v: on_speed_change(
            v,
            widgets,
            {"apply_stem_mix": apply_stem_mix, "invalidate_stem_caches": invalidate_stem_caches},
        ),
        bg=COLOR_BG,
        fg=COLOR_TEXT,
        troughcolor="#444",
    ...
    master_volume_slider.bind(
        "<Double-Button-1>", lambda e: reset_master_volume(master_volume_slider)
    )
    ...
    multi_loop_btn = tk.Button(
        ...
        command=lambda: toggle_multi_loop(
            multi_loop_btn, {"update_button_label": update_button_label}
        ),
        bg=COLOR_LOCK_OFF,
        fg=COLOR_TEXT,
        width=12,
    ...
    def create_stem_button(parent, stem_name, label):
        ...
        size = 32  # Durchmesser
        canvas = tk.Canvas(
            parent, width=size, height=size, bg=COLOR_BG, highlightthickness=0, cursor="hand2"
        )
    ...
    stem_label_text = tk.Label(
        stems_frame, text="STEMS:", fg="#888", bg=COLOR_BG, font=("Arial", 8)
    )
    ...
    def create_stop_stem_button(parent):
        ...
        canvas.create_text(
            size // 2, size // 2, text="S", fill=COLOR_TEXT, font=("Arial", 11, "bold"), tags="text"
        )
