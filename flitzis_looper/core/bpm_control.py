...
def detect_bpm(filepath, callback):
    ...
    def do_detect():
        try:
            ...
            logger.debug("BPM detection error: %s", e)
...
def detect_bpm_async(filepath, button_id, loop, callbacks):
    ...
    def do_detect():
        try:
            ...
            logger.debug("Async BPM detection error: %s", e)
        ...
        if bpm is not None:
            ...
            if button_data[button_id]["auto_loop_active"]:
                try:
                    ...
                    auto_loop_duration = bar_duration * button_data[button_id].get(
                        "auto_loop_bars", 8
                    )
                    ...
                    logger.debug("Auto-loop calculation error: %s", e)
...
def update_bpm_display(bpm_display_widget):
    ...
    if _has_active_loops:
        ...
        root.after(
            500, lambda: update_bpm_display(bpm_display_widget)
        )  # Langsameres Update wenn nichts spielt
