...
def _build_stem_mix_and_eq(button_id, loop, all_players):
    ...
    if all_players:
        ...
        eq_low = EQ(stem_mix, freq=200, q=0.7, boost=loop._get_eq_boost(loop._eq_low_val), type=1)
        eq_mid = EQ(eq_low, freq=1000, q=0.7, boost=loop._get_eq_boost(loop._eq_mid_val), type=0)
        eq_high = EQ(eq_mid, freq=4000, q=0.7, boost=loop._get_eq_boost(loop._eq_high_val), type=2)
...
def _select_main_audio_for_stems(loop, use_key_lock, current_speed, loop_start_time, loop_end_time):
    ...
    elif loop._audio_data is not None:
...
def initialize_stem_players(button_id):
    ...
    if need_new_tables:
        ...
        main_audio = _select_main_audio_for_stems(
            loop, use_key_lock, current_speed, loop_start_time, loop_end_time
        )
    ...
    if main_table:
        main_player = Pointer(table=main_table, index=master_phasor, mul=main_gain)
    ...
    for stem in STEM_NAMES:
        ...
        player = Pointer(table=table, index=master_phasor, mul=stem_gain)
