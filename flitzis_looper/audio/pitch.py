...
def precache_pitched_stems_if_needed(button_id):
    ...
    for stem in STEM_NAMES:
        if data["stems"]["dry"].get(stem) is not None and (
            data["stems"]["cached_speed"] != current_speed or data["stems"]["pitched"][stem] is None
        ):
            ...
            for stem in STEM_NAMES:
                ...
                if player:
                    with contextlib.suppress(builtins.BaseException):
                        player.stop()
            # Caches löschen
            data["stems"]["pitched"] = dict.fromkeys(STEM_NAMES)
            data["stems"]["players"] = dict.fromkeys(STEM_NAMES)
            data["stems"]["tables"] = dict.fromkeys(STEM_NAMES)
