...
from flitzis_looper.core.state import (
    get_root, get_speed_value, get_bpm_lock_active, get_key_lock_active,
    get_multi_loop_active, get_master_bpm_value, get_master_volume,
    COLOR_BG, COLOR_TEXT, COLOR_TEXT_ACTIVE,
    COLOR_LOCK_OFF, COLOR_LOCK_ON, COLOR_BTN_ACTIVE, COLOR_RESET_RED,
)
...
def create_toolbar(
    ...
    # BPM Display
    _bpm_display = tk.Label(right_frame, text="------", font=("Courier", 28, "bold"), 
    ...
    # KEY LOCK Button
    _key_lock_btn = tk.Button(lock_frame, text="KEY LOCK", command=toggle_key_lock_callback, 
                             bg=COLOR_LOCK_OFF, fg=COLOR_TEXT, width=12)
    _key_lock_btn.pack(pady=(0, 5))
    
    # BPM LOCK Button
    _bpm_lock_btn = tk.Button(lock_frame, text="BPM LOCK", command=toggle_bpm_lock_callback, 
                             bg=COLOR_LOCK_OFF, fg=COLOR_TEXT, width=12)
    ...
    _bpm_entry = tk.Entry(lock_frame, textvariable=master_bpm_value, width=12, 
...
def create_master_volume(on_master_volume_change_callback):
    ...
    _volume_label = tk.Label(volume_frame, text="Master Volume 100%", fg=COLOR_TEXT, bg=COLOR_BG, font=("Arial", 10))
    _volume_label.pack()
...
def update_reset_button_style():
    ...
    if current == 1.0 or abs(current - 1.0) < 0.001:
        _reset_btn.config(bg=COLOR_BTN_ACTIVE, fg=COLOR_TEXT_ACTIVE,
                        activebackground=COLOR_BTN_ACTIVE, activeforeground=COLOR_TEXT_ACTIVE)
    else:
        _reset_btn.config(bg=COLOR_RESET_RED, fg=COLOR_TEXT,
