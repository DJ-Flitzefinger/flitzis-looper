...
The system SHALL support placing loop markers with a granularity of one sample at the loaded sample rate.

When the waveform editor is zoomed such that individual samples are visible, setting loop start or loop end SHALL result in a marker time corresponding to an integer sample index at the loaded sample rate (i.e., `n / sample_rate_hz`).
...
- **THEN** the stored loop marker time corresponds to an integer sample index at the loaded sample rate
