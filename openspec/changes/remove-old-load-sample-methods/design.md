...
2. **Keep `sample_loader` for decoding**:
   - Continue using the existing `sample_loader` module for decoding/resampling work
   - Ensure `load_sample_async` uses `sample_loader` for file decoding
