...
- [ ] 1.4 Add waveform editor window in ImGui:
  ...
  - [ ] 1.4.3 Mouse: wheel zoom; middle-drag pan; left click set loop start; right click set loop end (only when auto-loop off).
  - [ ] 1.4.4 Visuals: waveform (mono), loop region shaded light yellow, start line blue, end line red, playhead marker.
  - [ ] 1.4.5 Render waveform with ImPlot (from `imgui_bundle`) using a cached envelope at normal zoom.
  - [ ] 1.4.6 At extreme zoom, render individual samples and allow sample-accurate marker placement.
- [ ] 1.5 Add/extend tests:
  - [ ] 1.5.1 Python unit tests for loop-region defaults, auto-loop math, snapping behavior.
  - [ ] 1.5.2 Rust unit tests for loop wrap math and message handling (where possible).

## 2. Validation
- [ ] 2.1 Run Python checks: `ruff`, `mypy`, `pytest`.
- [ ] 2.2 Run Rust checks: `cargo test` (and `cargo fmt`/`cargo clippy` if available in repo workflow).
