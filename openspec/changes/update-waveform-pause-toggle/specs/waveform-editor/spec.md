...
- Pause: icon is two vertical bars; toggles playback: if playing, pauses immediately without resetting playhead; if paused, resumes playback from the paused position.
...
#### Scenario: Pause button toggles between pause and resume
- **GIVEN** Pad A is selected and playing
- **WHEN** the performer presses Pause in the waveform editor (mouse-down)
- **THEN** Pad A playback pauses immediately and the playhead remains at the paused position
- **WHEN** the performer presses Pause again (mouse-down)
- **THEN** Pad A playback resumes from the paused position
