...
### Requirement: Transport Phase Supports Pad-Aligned Target Frames
...
relative to the transport downbeat anchor. This phase SHALL be usable by playback code to
choose a phase-aligned initial pad sample frame.
...
#### Scenario: Missing master BPM disables phase-aware target calculation
- **GIVEN** no valid master BPM is stored in the transport
- **WHEN** Rust evaluates a scheduled playback target frame
- **THEN** no phase-aware transport target is produced
- **AND** the audio callback continues without panic, blocking, allocation, disk I/O, logging, or GIL access

### Requirement: BPM Lock Can Anchor Transport Downbeat From A Playing Pad
When BPM lock is enabled and a selected anchor pad is active, the system SHALL support
anchoring the Rust transport downbeat to the anchor pad's current musical phase.
...
transport downbeat unchanged and SHALL preserve existing BPM-lock tempo matching behavior.

#### Scenario: Playing anchor pad sets transport phase
- **GIVEN** BPM lock is enabled with a valid master BPM
- **AND** the selected anchor pad is playing
- **AND** the anchor pad has valid BPM and phase-anchor metadata
