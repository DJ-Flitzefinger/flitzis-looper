...
octave-family consensus than the selected primary BPM, or when decoded audio contains enough strong
transient starts that fit one stable constant-tempo grid near the chosen candidate-family BPM. The
fixed-tempo refinement SHALL publish the chosen BPM rounded to 0.01 BPM and SHALL leave the primary
analysis BPM unchanged when candidate-family consensus is inconclusive and the transient grid is
missing, sparse, unstable, or inconsistent with the chosen tempo.
