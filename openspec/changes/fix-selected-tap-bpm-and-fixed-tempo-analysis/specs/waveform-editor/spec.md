...
- The grid MUST be anchored at `grid_anchor_sec`, defined as:
  ...
  - `default_onset_sec` follows "Default loop region uses analysis downbeat onset" in `loop-region`, including snapping first beat/downbeat values very close to file start to `0.0` seconds.
...
#### Scenario: Near-start analysis anchor places the first bar line at track start
- **GIVEN** an effective BPM is available
- **AND** analysis reports the first downbeat very close to `0.0` seconds
- **AND** the pad's Grid Offset is 0 samples
- **WHEN** the waveform editor renders the fully zoomed-out musical grid
- **THEN** the grid anchor is `0.0` seconds
- **AND** the first bar-aligned major line is placed at track start
